...
program
    ...
    .option('-o, --output [outputFile]',
        'Output file for downloaded course schedules')
    .option('--show-term',
        'Only display the selected `term` and do not continue to download')
    .option('--term [term]',
        'Force the selected `term`')
    .parse(process.argv);

if (!(program.output) & !(program.showTerm)) {
    console.error('Error: CLI requires output file paths.');
    process.exit();
    return;
}

function downloadTerm(termv, banner) {
    ...
    banner.getSubjects({
        ...
        var taskFun = function(subject, termv) {
            ...
            var task = function(callback) {
                ...
                banner.getCoursesSchedule({
                    ...
                    courses) {
                    bar.fmt =
                        '[:bar] Finished downloading ' +
                        subject
                        .title +
                        ': :percent (:current of :total subjects downloaded)';
                    bar.tick(1);
...
if (program.term) {
    downloadTerm(program.term, banner);
} else {
    ...
    banner.getTerms(function(error, response, terms) {
        ...
        inquirer.prompt(questions, function(answers) {
            ...
            if (program.showTerm) {
                console.log("Term: " + termv);
                return;
            }

            downloadTerm(termv, banner);
