...
Main
*/
MongoClient.connect('mongodb://' + nconf.get('database:hostname') + ':' + nconf
    .get('database:port') + '/' + nconf.get('database:name'),
    function(err, db) {
        if (err) throw err;
        ...
        */
        ...
        var saveCalendar = function(calendarId, courses, collection,
            callback) {
            var data = {
        ...
        app.post('/api/calendar', function(req, res) {
            ...
            if (courses.length > 0) {
                ...
                // console.log(temp);
                var digest = "hex";
                var calendarId = crypto.createHash('md5').update(
                    temp).digest(digest);
                // console.log(calendarId);

                saveCalendar(calendarId, courses, collection,
                    function(err, docs) {
                        // console.log('Saved Courses');

                        var url = "/calendar/" + calendarId +
                            "/calendar.ics";
                        res.json({
                            'url': url,
                            'courses': courses
        ...
        var jsonToEvent = function(json) {
            ...
            var summary = json.Subj_code + " " + json.Crse_numb + " - " +
                json.Crse_title;
        ...
        var reloadCourses = function(courses, callback) {
            ...
            async.map(courses, function(course, cb) {
                var baseurl = nconf.get('uniapi:protocol') +
                    "://" + nconf.get('uniapi:hostname') + ":" +
                    nconf.get('uniapi:port');
                var url = baseurl + "/api/v1/courses/" + course
                    ._id;
        ...
        app.listen(process.env.PORT || nconf.get('server:port'), function() {
            console.log('iCal-Sync Server is listening on ' + nconf
                .get('server:port') + '.');
        });
